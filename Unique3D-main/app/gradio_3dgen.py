...
# 设置 Gradio 使用自定义临时目录（而不是 /tmp/gradio）
os.environ['GRADIO_TEMP_DIR'] = './gradio_temp'  # 或使用 ./gradio_outputs
os.makedirs(os.environ['GRADIO_TEMP_DIR'], exist_ok=True)

# 也可以设置静态文件目录
gr.set_static_paths(["./gradio_temp"])
...
def generate3dv2(preview_img, input_processing, seed, render_video=True, do_refine=True, expansion_weight=0.1, init_type="std"):
    ...
    ret_mesh, video = save_glb_and_video("./gradio_outputs", new_meshes, with_timestamp=True, dist=3.5, fov_in_degrees=2 / 1.35, cam_type="ortho", export_video=render_video)
