...
# ======== 关键修复：Monkey Patch trimesh 导出函数 ========
original_export = trimesh.exchange.export.export_mesh

@wraps(original_export)
def safe_export_mesh(mesh, file_obj=None, file_type=None,** kwargs):
    """覆盖trimesh的导出函数，确保使用安全路径"""
    if isinstance(file_obj, str):
        # 创建用户目录
        user_dir = Path("./gradio_outputs").resolve()
        user_dir.mkdir(exist_ok=True, parents=True)
        
        # 如果是/tmp/gradio路径，替换为用户目录
        if "/tmp/gradio/" in file_obj:
            filename = Path(file_obj).name
            file_obj = str(user_dir / filename)
        else:
            # 确保所有文件都保存在gradio_outputs目录
            file_obj = str(user_dir / Path(file_obj).name)
    
    return original_export(mesh, file_obj=file_obj, file_type=file_type, **kwargs)

# 应用patch
trimesh.exchange.export.export_mesh = safe_export_mesh
trimesh.base.Trimesh.export = lambda self, file_obj=None, file_type=None,** kwargs: safe_export_mesh(self, file_obj=file_obj, file_type=file_type, **kwargs)

# 添加全局路径处理函数
def force_safe_path(file_path):
    """强制将路径转换为用户可写的路径"""
    # 创建用户目录
    user_dir = Path("./gradio_outputs").resolve()
    user_dir.mkdir(exist_ok=True, parents=True)
    ...
    # 如果是/tmp/gradio路径，替换为用户目录
    if "/tmp/gradio/" in str(file_path):
...
def fix_vert_color_glb(mesh_path):
    ...
    # 确保使用安全路径
    mesh_path = force_safe_path(mesh_path)
...
def save_py3dmesh_with_trimesh_fast(meshes: Meshes, save_glb_path, apply_sRGB_to_LinearRGB=True):
    # 强制使用安全路径
    save_glb_path = force_safe_path(save_glb_path)
    ...
    # save mesh（现在会通过monkey patch自动使用安全路径）
...
def save_glb_and_video(save_mesh_prefix: str, meshes: Meshes, with_timestamp=True, dist=3.5, azim_offset=180, resolution=512, fov_in_degrees=1 / 1.15, cam_type="ortho", view_padding=60, export_video=True) -> Tuple[str, str]:
    ...
    # 使用安全路径
    save_mesh_prefix = force_safe_path(save_mesh_prefix)
        
    # 分离目录和文件名前缀
    save_dir = os.path.dirname(save_mesh_prefix)
    filename_prefix = os.path.basename(save_mesh_prefix)
    
    # 如果传入的是目录，使用默认文件名前缀
    if filename_prefix == "" or os.path.isdir(save_mesh_prefix):
        filename_prefix = "generated"
        save_dir = "./gradio_outputs"
    ...
    # 生成文件名
    if with_timestamp:
        full_prefix = os.path.join(save_dir, f"{filename_prefix}_{int(time.time())}")
    else:
        full_prefix = os.path.join(save_dir, filename_prefix)
    
    ret_mesh = full_prefix + ".glb"
    
    # 再次确保使用安全路径
    ret_mesh = force_safe_path(ret_mesh)
...
def init_target(img_pils: List[Image.Image], new_bkgd: Tuple[float, float, float] = (0., 0., 0.), device: str = "cuda") -> torch.Tensor:
    """
    Initialize target images with new background
    
    Args:
        img_pils: List of PIL images with alpha channel
        new_bkgd: New background color (RGB)
        device: Device to use (cuda/cpu)
    
    Returns:
        Tensor of processed images with shape [N, H, W, 4]
    """
    if not img_pils:
        raise ValueError("img_pils list is empty")
    ...
    try:
        ...
        img_nps = torch.where(alpha_nps.unsqueeze(-1) > 0.05, 
                             ori_img_nps * alpha_nps.unsqueeze(-1) + new_bkgd * (1 - alpha_nps.unsqueeze(-1)), 
                             new_bkgd)
        ...
        print(f"Error in init_target: {e}")
        raise